...
from data_utils import load_dialog_task, vectorize_data, load_candidates, \
    vectorize_seq2seq_fix, tokenize, vectorize_seq2seq, vectorize_candidates, vectorize_attnNew
import metrics
from memn2n.attentionn2n_dialogNew import AttentionN2NDialogNew
import datetime
...
class chatBot(object):
    def __init__(self, data_dir, model_dir, task_id, isInteractive=True, random_state=None,memory_size=50,
        ...
        self.model = AttentionN2NDialogNew(self.batch_size, self.vocab_size, self.sentence_size, self.embedding_size,
                                        blocks=2, num_heads=8, dropout_rate=0.1,
                                        max_grad_norm=40.0,
                                        nonlin=None,
                                        optimizer=optimizer,
                                        session=self.sess,
    ...
    def train(self):
        trainS,trainQ,trainA = vectorize_attnNew(
            self.trainData, self.word_idx, self.sentence_size, self.candidate_sentence_size, self.memory_size)
        valS, valQ, valA = vectorize_attnNew(
            self.valData, self.word_idx, self.sentence_size, self.candidate_sentence_size, self.memory_size)
        ...
        for t in range(1, self.epochs + 1):
            ...
            if t % self.evaluation_interval == 0:
                train_preds = self.batch_predict(trainS[:30], trainQ[:30], 30)
                val_preds = self.batch_predict(valS, valQ, n_val)
                self.sample_output(valS[:30], valQ[:30], valA[:30], val_preds[:30])
                # write summary
    ...
    def test(self):
        ...
        if self.isInteractive:
            ...
            testS, testQ, testA = vectorize_attnNew(
                self.testData, self.word_idx, self.sentence_size, self.candidate_sentence_size, self.memory_size)
            n_test = len(testS)
            print("Testing Size", n_test)
            test_preds = self.batch_predict(testS, testQ, n_test)
            self.sample_output(testS[:30], testQ[:30], testA[:30], test_preds[:30])
            self.eval_acc(test_preds, testA, 'Testing')

    def sample_output(self, H, Q, S, A):
        for h, q, s, a in zip(H, Q, S, A):
            h = [self.idx_word[idx] for idx in h]
            q = [self.idx_word[idx] for idx in q]
            s = [self.idx_word[idx] for idx in s]
            a = [self.idx_word[idx] for idx in a]
            print('--History--:%s\n--Query--:%s\n--Answer--:%s\n--Got--:%s\n\n' % (' '.join(h), ' '.join(q), ' '.join(s), ' '.join(a)))
